/**
 * Sector Stocks Page - Ant Design Implementation
 * Shows stocks within a selected sector
 */
...
import { 
  ReloadOutlined, AppstoreOutlined, ArrowUpOutlined, 
  ArrowDownOutlined, MinusOutlined 
} from '@ant-design/icons'
import { getSectorStocks, getSectorList } from '../api/scanner'
...
const SectorStocks = () => {
    ...
    const fetchSectors = async () => {
      try {
        ...
        if (result?.sectors?.length > 0) {
          setSelectedSector(result.sectors[0].name)
        }
      } catch (err) {
  ...
  const fetchData = async () => {
    ...
    setError(null)
    try {
      const result = await getSectorStocks(selectedSector, 'weekly', lookback)
      setData(result)
    } catch (err) {
      setError(err.message || 'Failed to fetch sector stocks data')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div>
        ...
        <Col>
          <Space align="center">
            ...
            <div>
              ...
              <Text type="secondary">
                Individual stocks within a sector vs NIFTY 50
                {data?.stocks && (
        ...
        <Row gutter={[16, 16]} align="middle">
          <Col xs={24} sm={12} md={8}>
            <Space direction="vertical" size={4} style={{ width: '100%' }}>
              <Text type="secondary" style={{ fontSize: 12 }}>SELECT SECTOR</Text>
              <Select
        ...
        <Card>
          <div style={{ textAlign: 'center', padding: 60 }}>
            ...
            <div style={{ marginTop: 16 }}>
              <Text type="secondary">Loading {selectedSector} stocks...</Text>
            </div>
          </div>
        </Card>
      )}
      ...
      {!loading && !error && (!data?.stocks || data.stocks.length === 0) && (
        <Card>
          <Empty description={selectedSector ? "No stocks data available" : "Select a sector to view stocks"} />
        </Card>
      )}
