/**
 * Bank Nifty Heavyweight Stocks Page
 * Shows major banking stocks that drive Bank Nifty index movement with their weightage
 */

import React, { useState } from 'react';
import { getBankNiftyHeavyweights } from '../api/scanner';
...
const BankNifty = () => {
  ...
  return (
    <div className="page banknifty-page">
      <div className="page-header">
        <h2>Bank Nifty Heavyweight Stocks</h2>
        <p className="page-desc">
          Major banking stocks relative comparison for understanding Bank Nifty price movement
        </p>
      </div>
      ...
      {loading && <Loader message="Fetching Bank Nifty heavyweight data..." />}
      ...
      {!loading && !error && data && (
        <>
          ...
          <div className="table-container">
            <table className="data-table banknifty-table">
              <thead>
                <tr>
                  <th>#</th>
                  <th>Bank</th>
                ...
                {data.stocks?.map((stock, index) => (
                  <tr key={stock.symbol}>
                    ...
                    <td className="weightage">
                      <span className="weightage-bar" style={{width: `${Math.min(stock.weightage * 3, 100)}%`}}></span>
      ...
      {!loading && !error && !data && (
        <div className="no-data">
          Click Refresh to load Bank Nifty heavyweight stocks data
        </div>
      )}
    </div>
  );
