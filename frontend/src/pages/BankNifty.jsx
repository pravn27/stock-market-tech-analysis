/**
 * Bank Nifty Page - Ant Design Implementation
 * Shows Bank Nifty stocks performance
 */
...
import { 
  ReloadOutlined, BankOutlined, ArrowUpOutlined, 
  ArrowDownOutlined, MinusOutlined 
} from '@ant-design/icons'
import { getBankNiftyStocks } from '../api/scanner'
...
const BankNifty = () => {
  ...
  const fetchData = async () => {
    ...
    setError(null)
    try {
      const result = await getBankNiftyStocks(lookback)
      setData(result)
    } catch (err) {
      setError(err.message || 'Failed to fetch Bank Nifty data')
    } finally {
      setLoading(false)
    }
  }
        ...
        <Col>
          <Space align="center">
            ...
            <div>
              ...
              <Text type="secondary">
                Banking sector stocks relative strength vs NIFTY 50
                {data?.stocks && (
        ...
        <Card>
          <div style={{ textAlign: 'center', padding: 60 }}>
            ...
            <div style={{ marginTop: 16 }}>
              <Text type="secondary">Loading Bank Nifty stocks...</Text>
            </div>
          </div>
        </Card>
      )}
