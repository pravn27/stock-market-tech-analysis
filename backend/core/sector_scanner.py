...
# Sectorial Indices (All sectors with constituent stock data)
NIFTY_SECTORS_MAIN = {
    "NIFTY 50": "^NSEI",
...
# Broader Market Indices (aggregated indices without constituent stock data)
NIFTY_BROAD_INDICES = {
    # Benchmark
    "NIFTY 50": "^NSEI",
    
    # Broad Market Aggregates
...
class SectorRelativeStrength:
    ...
    def calculate_returns(self, df: pd.DataFrame, lookback: int = 1) -> Dict[str, float]:
        ...
        if len(df) > daily_offset:
            # Use the last available complete day
            # yfinance with interval='1d' typically only includes complete days
